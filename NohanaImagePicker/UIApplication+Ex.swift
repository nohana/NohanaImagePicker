...
extension UIApplication {
    var currentStatusBarOrientation: UIInterfaceOrientation {
        if let orientation = windows.first(where: { $0.isKeyWindow })?.windowScene?.interfaceOrientation {
            return orientation
        }
        return .unknown
    }
    ...
    var currentStatusBarFrame: CGRect {
        if let frame = windows.first(where: { $0.isKeyWindow })?.windowScene?.statusBarManager?.statusBarFrame {
            return frame
        }
        return .zero
    }
